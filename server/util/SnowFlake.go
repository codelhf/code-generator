 ...
 * 1bit sign | 41bit timestamp | 10 bit machineID : 5bit workerID 5bit dataCenterID ｜ 12 bit sequenceBits <br>
 * 1位标识，由于long基本类型在Java中是带符号的，最高位是符号位，正数是0，负数是1，所以id一般是正数，最高位是0<br>
 * 41位时间截(毫秒级)，注意，41位时间截不是存储当前时间的时间截，而是存储时间截的差值（当前时间截 - 开始时间截)
 * 得到的值），这里的是开始时间截，一般是我们的id生成器开始使用的时间，由我们程序来指定的（如下的epoch属性）。
...
func (w *SnowflakeIdWorker) NextId() int64 {
	...
	w.lastTimestamp = now
	return (now-twepoch)<<timestampLeftShift | w.dataCenterId<<dataCenterIdShift | (w.workerId<<workerIdShift) | w.sequence
}
